...
resource "aws_s3_bucket_public_access_block" "static_website" {
  # Block Public Acces bezieht sich auf Bucket oben
...
resource "aws_s3_bucket_policy" "static_website_public_read" {
  ...
  policy = jsonencode({
    # erlaubt Public Read (get object from bucket, aber keine anderen Aktionen)
    # jeder kann von diesem Bucket lesen (*)
    Version = "2012-10-17"
    Statement = [
      {
